    ...
    <div class="modal-dialog">
        <form action="{{ route('clients.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="row g-3">
                    <div class="col-8">
                        <label class="fw-bold form-label" for="create_document">DNI / RUC</label>
                        <div class="input-group">
                            <input type="text" name="document_number" id="create_document" class="form-control" placeholder="Ej: 12345678" inputmode="numeric" maxlength="11" autocomplete="off">
                            <button type="button" class="btn btn-outline-info" id="btnSearchDoc" title="Buscar en SUNAT">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function searchDocument(event) {
            ...
            fetch("{{ route('clients.searchDocument') }}?document=" + encodeURIComponent(document.getElementById('create_document').value))
                .then(res => res.json().then(data => ({ ok: res.ok, data })))
                .then(({ ok, data }) => {
                    btn.disabled = false;
                    icon.className = 'bi bi-search';
                    if (!ok) {
                        const link = data.link ? ' <a href="' + data.link + '" target="_blank" rel="noopener">Consultar en SUNAT</a>' : '';
                ...
                .catch(err => {
                    btn.disabled = false;
                    icon.className = 'bi bi-search';
                    feedback.textContent = 'Error al consultar. Inténtalo de nuevo.';
                    feedback.className = 'form-text text-danger';
                });
        }

        btn.addEventListener('click', searchDocument);
        document.getElementById('create_document').addEventListener('keydown', function (e) {
            if (e.key === 'Enter') { e.preventDefault(); searchDocument(); }
        });
    });
</script>
@endsection